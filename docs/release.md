...
(cd dist && shasum -a 256 -c Tomito-vlv-1.0.0-macos-unsigned.zip.sha256)
...
- `dist/Tomito-vlv-1.0.0-macos-unsigned.zip`
- `dist/Tomito-vlv-1.0.0-macos-unsigned.zip.sha256`
...
This creates local-only files in `dist/sourceforge/v1.0.0/`. See [SourceForge mirror](sourceforge.md) for the manual upload list.
